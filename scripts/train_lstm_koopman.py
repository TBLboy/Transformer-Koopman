"""Train the LSTM-Koopman baseline (temporal comparison vs PatchTST)."""
import argparse
import json
import os
...
from patchtst_koopman.training.lstm_koopman_trainer import LSTMKoopmanTrainer
...
def parse_args():
    parser = argparse.ArgumentParser(description="LSTM-Koopman baseline training")
    parser.add_argument("--config", type=str, required=True, help="YAML config path")
    parser.add_argument(
        "--save_dir",
        type=str,
        default="./results/lstm_koopman",
...
def main():
    ...
    print("=" * 60)
    print("LSTM-Koopman training (temporal baseline)")
    print("=" * 60)
    ...
    trainer = LSTMKoopmanTrainer(model, config)
    ...
    with open(os.path.join(args.save_dir, "results.json"), "w") as f:
        json.dump(
            {
                "method": "lstm_koopman",
                "training_method": config["lstm_koopman"]["training_method"],
